...
$CATAL
### EARNINGS GATE (ot earnings — held/watched names reporting soon; take the call BEFORE the print)
$EARN
...
3. <h2>Holdings — levels &amp; call</h2> — a <table>: columns Name | Last | Day | Call | Levels (buy · trim · stop) | Read.
   One row per HELD name: in the Name column use the DISPLAY NAME shown in the range-plan header (for an A-share/HK code, show the company name, never the bare code alone); tk ticker cell, num Last/Day cells, an action badge in Call, the mechanical zones FROM THE DATA, a one-line read.
4. <h2>Alpha Watch</h2> (titled "Watchlist — Top 3") — the candidate pool may hold many names, but DO NOT give every candidate a row. RANK them and feature ONLY THE TOP 3 FOR TODAY (best grade / closest to buy zone / best fit for TODAY's regime) as <table> rows: Name (+theme sub-line) | Last | Day | <span class="grade">A/B/C/D</span> | Bull vs Bear | Action. For EACH of the 3: a one-line stress-test (one <span class="bull">Bull</span> line, one <span class="bear">Bear</span> line), a real letter grade, and an EXPLICIT action badge — either <span class="buy">Enter</span> when Last is AT/INSIDE its buy zone NOW, or <span class="watch">Wait</span> WITH THE EXACT buy-zone price to wait for (e.g. "wait for a pullback to 14.50–15.49"); never a vague watch with no number. Then ONE short <p> naming the remaining candidates as "also watching, not today" — no rows for them. (Omit this section entirely if there is no candidate data.)
...
Be earnings-aware: if a held or watched name reports earnings within the window — check the news digest and the
