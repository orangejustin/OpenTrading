...
<div style="color:#ffffff;font-size:16px;font-weight:700;letter-spacing:.2px;">OpenTrading&nbsp;<span style="color:#60a5fa;font-weight:600;">· {header}</span></div>
...
def main():
    ...
    ap.add_argument("--lang", default="en", help="en | zh — language of the header/footer chrome")
    ap.add_argument("--header", default="", help="override the header label (e.g. '盘前策略 · A股/港股')")
    a = ap.parse_args()
    ...
    zh = a.lang.lower() in ("zh", "cn", "zh-cn", "zh_cn", "chinese")
    header = a.header or ("盘前速递" if zh else "Pre-Market Read")
    footer = (
        '由 OpenTrading 生成 — 宏观优先、风险优先。数据源：FinancialJuice（新闻）、政府宏观、'
        'CNN/加密 Fear&amp;Greed、CBOE 期权链、Yahoo 报价、Coinbase BTC。'
        '<strong style="color:#64748b;">仅供教育，非投资建议。</strong>'
        if zh else
        'Generated by OpenTrading — macro-first, risk-first. Sources: FinancialJuice (24h news), '
        'gov macro, CNN/crypto Fear&amp;Greed, CBOE option chains, Yahoo quotes, Coinbase BTC. '
        '<strong style="color:#64748b;">Educational only — not financial advice.</strong>'
    )
    full = SHELL.format(date=_html.escape(a.date), body=frag, header=header, footer=footer)
